...
public class FileGen implements Generator {
    ...
    private void initFieldsFromProperties() {
        ...
        String path = "src/main/java/lesson10/task3/application.properties";
        try {
            properties.load(new FileReader(path));
        } catch (IOException e) {
            e.printStackTrace();
        }
        // »нициализируем значени€, которые не были инициализированы в конструкторе
        if (amount == null)
    ...
    private void initListField(List<Object> objects) {
        ...
        if (objects.isEmpty()) throw new IllegalArgumentException("ѕередан пустой список");
